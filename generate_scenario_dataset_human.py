...
class HumanPlayedDatasetGenerator:
    ...
    def play_game_human(self, game: str):
        ...
        # Initialize OCAtari with human rendering
        ocatari = OCAtariGroundTruth(game, render_mode='human')
        ocatari.reset()

        frame_count = 0
        check_interval = 5  # Check every N frames to reduce overhead

        # Import pygame for keyboard handling
        import pygame
        pygame.init()

        # Create small control window for keyboard input
        control_screen = pygame.display.set_mode((400, 200))
        pygame.display.set_caption(f"Controls - {game}")
        ...
        try:
            ...
            while running and scenarios_needed:
                ...
                # Update control window with progress
                control_screen.fill((30, 30, 30))

                # Draw title
                title = font.render(f"{game} Controls", True, (255, 255, 0))
                control_screen.blit(title, (120, 10))

                # Draw progress
                progress_text = f"Collected: {len(self.collected_scenarios[game])}/{len(scenarios)}"
                progress = font.render(progress_text, True, (0, 255, 0))
                control_screen.blit(progress, (120, 45))
                ...
                for hint in hints.get(game, ['Use arrow keys']):
                    ...
                    control_screen.blit(text, (80, y))
                    y += 25

                # Draw exit hint
                exit_text = font.render("Close window to stop", True, (150, 150, 150))
                control_screen.blit(exit_text, (100, 170))

#!/usr/bin/env python3
"""
Run spatial control benchmark on both pipelines.
...
def run_spatial_control_benchmark(
    ...
    """Run spatial control tests on a pipeline."""
    ...
    for frame_data in tqdm(frames, desc=f"Running {pipeline_name}"):
        ...
        test = evaluator.create_movement_distance_test(ground_truth)
        if not test:
            continue
    ...
    print(f"  Perfect Predictions: {results['statistics']['perfect_predictions']}/{len(results['tests'])}")
...
def compare_results(baseline_file: Path, comparison_file: Path):
    ...
    print(f"{comparison['pipeline_name']:20} → Score: {comparison_score:.3f}")
...
def main():
    parser = argparse.ArgumentParser(description='Run spatial control benchmark')
    ...
    if args.pipeline == 'both':
        compare_results(
            ...
            Path(args.output) / 'vision_symbol_spatial_control.json'
        )

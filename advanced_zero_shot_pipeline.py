...
class AdvancedSymbolicDetector:
    def __init__(self, openrouter_api_key: str, model_name: str = "anthropic/claude-sonnet-4", detection_mode: str = "specific", provider: str = "openrouter", aws_region: str = "us-east-1"):
        ...
        self.conversation_history = []  # For memory module
        self.planning_memory = []  # For storing planning steps
...
"""
    ...
    def plan_next_action(self, symbolic_state: Dict, game_name: str, image_path: str) -> str:
        """
        Planning module - generate a strategic plan based on the current state and visual frame.
        """
        # Save previous planning steps
        if len(self.planning_memory) > 5:  # Keep only last 5 planning steps
            self.planning_memory.pop(0)

        base64_image = self.encode_image_to_base64(image_path)
            
        plan_prompt = f"""You are an expert {game_name} player. Analyze the provided game frame and the accompanying symbolic data to create a strategic plan for the next few moves.\n\nThe symbolic data is generated by an object detection model and may be imperfect. Use the image as the primary source of truth and the symbolic data as a reference.\n\nSymbolic Data:\n- Total objects detected: {symbolic_state.get("total_objects", 0)}\n"""
        
        # Add object positions
        for obj in symbolic_state.get("objects", []):
            plan_prompt += f"- Object '{obj['label']}': coordinates at x={obj['x']}, y={obj['y']}\n"
        
        plan_prompt += """'WARNING: Trust the visual frame more than any symbolic data. Symbolic detection can be incorrect.\n'
 \nPlease provide a strategic plan that includes:\n1.  **Objective**: What is the primary objective in this game state?\n2.  **Threats/Opportunities**: What are the immediate threats or opportunities visible in the frame?\n3.  **Next Actions**: What should be the next 2-3 actions to achieve the objective?\n4.  **Considerations**: Any special considerations based on the game rules?\n\nReturn ONLY valid JSON in the following format:\n{\n    "plan": "your strategic plan",\n    "objective": "primary objective",\n    "threats_opportunities": "key threats and opportunities",\n    "next_actions": ["action1", "action2", "action3"]\n}\n"""
        ...
        response = self._make_api_call(messages, max_tokens=1000, call_id="planning")
        if not response:
            return "Failed to generate plan"
            
        # Save the full response for debugging
        self._last_plan_response = response
        ...
        try:
            ...
            self.planning_memory.append({
                ...
                "plan": plan_text,
                "objective": plan_result.get("objective", ""),
                "next_actions": plan_result.get("next_actions", [])
            })
            ...
            if json_matches:
                print(f"Found JSON-like strings: {json_matches}")
            return "Error generating plan"
    
    def generate_simple_action_prompt(self, symbolic_state: Dict, game_name: str, game_controls: Dict) -> str:
        """
        Generate a simple action prompt without memory or planning complexity
        """
        # Build controls text
        controls_text = ""
...
"""
    ...
    def generate_action_prompt_with_memory(self, symbolic_state: Dict, game_name: str, game_controls: Dict) -> str:
        ...
        if self.conversation_history:
            ...
            for i, interaction in enumerate(self.conversation_history[-3:]):  # Last 3 interactions
                prompt += f"- Previous decision {i+1}: {interaction['reasoning']}\n"
        ...
        if hasattr(self, 'memory_retriever') and self.memory_retriever:
            # Create a query from the current state
            query_parts = []
            query_parts.append(f"Current game: {game_name}")
            query_parts.append(f"Objects detected: {symbolic_state.get('total_objects', 0)}")
            for obj in symbolic_state.get("objects", [])[:3]:  # First 3 objects
                query_parts.append(f"Object: {obj['label']} at ({obj['x']}, {obj['y']})")
            
            query = " ".join(query_parts)
            long_term_memories = self.retrieve_long_term_memories(query, top_k=3)
            
            if long_term_memories:
                prompt += f"""\nRelevant Past Experiences (from long-term memory):\n"""
                for i, memory in enumerate(long_term_memories):
                    prompt += f"- {memory}\n"
        
        # Add planning memory if available
        if self.planning_memory:
            prompt += f"""\nRecent Strategic Plan:\n"""
            for i, plan in enumerate(self.planning_memory[-1:]):  # Last plan
                prompt += f"- Objective: {plan.get('objective', 'No objective specified')}\n"
                prompt += f"- Plan: {plan['plan']}\n"
        
        prompt += f"""'WARNING: Trust the visual frame more than any symbolic data. Symbolic detection can be incorrect.\n'
 \nBased on the visual evidence in the image and all the contextual information provided, choose the best action. Your primary guide should be the image itself.\n\nReturn ONLY valid JSON in the following format:\n{{\n    "reasoning": "Your expert analysis and decision rationale, citing visual evidence.",\n    "action": integer_action_code\n}}\n"""
    ...
    def set_memory_retriever(self, memory_retriever):
        """
        Set a function to retrieve long-term memories
        """
        self.memory_retriever = memory_retriever
    
    def retrieve_long_term_memories(self, query: str, top_k: int = 3) -> List[str]:
        """
        Retrieve long-term memories if a retriever is set
        """
        if hasattr(self, 'memory_retriever') and self.memory_retriever:
            return self.memory_retriever(query, top_k)
        return []
...
def main():
    ...
    parser.add_argument("--game", choices=["breakout", "frogger", "space_invaders", "pacman", "mspacman"], required=True, 
                        help="Game type")
